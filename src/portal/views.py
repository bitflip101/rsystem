...
def home_view(request):
    ...
    if request.user.is_authenticated:
        ...
        # User getattr to safely check for employee
        employee = getattr(request.user, 'employee', None)

        if employee and employee.position:
            context['user_asset'] = employee.position.org_unit.asset
            context['pending_count'] = WellProject.objects.filter(
                assigned_to_position = employee.position,
                status = 'IN_PROGRESS'
            ).count()
        else:
            # If they are logged in but have no 'Seat' yet (like a new superuser)
            context['pending_count'] = 0
            context['needs_setup'] = True # Use this in HTML to show a "Contact Admin"
